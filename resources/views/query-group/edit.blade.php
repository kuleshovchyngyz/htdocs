...
<div class="content-wrapper container-fluid">
    <div class="row mt-5">
        <div class="col-lg-8 offset-md-2">
            <div class="card">
                <div class="card-header"><h1>{{ __('Edit Query Group') }}</h1></div>

                <div class="card-body">
                    <form method="POST" action="{{ route('query-group.update', $item->id) }}">
                        @csrf
                            ...
                            <div class="col-md-6">
                                <select class="custom-select @error('parent_group_id') is-invalid @enderror" name="parent_group_id">
                                    ...
                                    @foreach ($query_groups as $query_group)
                                        <option value="{{ $query_group['id'] }}" {{$item->parent_group_id == $query_group['id'] ? 'selected' : ''}}>@php echo str_repeat('&nbsp;&nbsp;', $query_group['level']); @endphp {{ $query_group['name'] }}</option>
                                    @endforeach
                                </select>
